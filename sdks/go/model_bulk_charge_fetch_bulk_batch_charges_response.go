/*
Paystack

The OpenAPI specification of the Paystack API that merchants and developers can harness to build financial solutions in Africa.

API version: 1.0.0
Contact: techsupport@paystack.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package paystack

import (
	"encoding/json"
)

// checks if the BulkChargeFetchBulkBatchChargesResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BulkChargeFetchBulkBatchChargesResponse{}

// BulkChargeFetchBulkBatchChargesResponse struct for BulkChargeFetchBulkBatchChargesResponse
type BulkChargeFetchBulkBatchChargesResponse struct {
	Status bool `json:"status"`
	Message string `json:"message"`
	Data []BulkChargeFetchBulkBatchChargesResponseArray `json:"data"`
	Meta BulkChargeFetchBulkBatchChargesResponseMeta `json:"meta"`
}

// NewBulkChargeFetchBulkBatchChargesResponse instantiates a new BulkChargeFetchBulkBatchChargesResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBulkChargeFetchBulkBatchChargesResponse(status bool, message string, data []BulkChargeFetchBulkBatchChargesResponseArray, meta BulkChargeFetchBulkBatchChargesResponseMeta) *BulkChargeFetchBulkBatchChargesResponse {
	this := BulkChargeFetchBulkBatchChargesResponse{}
	this.Status = status
	this.Message = message
	this.Data = data
	this.Meta = meta
	return &this
}

// NewBulkChargeFetchBulkBatchChargesResponseWithDefaults instantiates a new BulkChargeFetchBulkBatchChargesResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBulkChargeFetchBulkBatchChargesResponseWithDefaults() *BulkChargeFetchBulkBatchChargesResponse {
	this := BulkChargeFetchBulkBatchChargesResponse{}
	return &this
}

// GetStatus returns the Status field value
func (o *BulkChargeFetchBulkBatchChargesResponse) GetStatus() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *BulkChargeFetchBulkBatchChargesResponse) GetStatusOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *BulkChargeFetchBulkBatchChargesResponse) SetStatus(v bool) {
	o.Status = v
}

// GetMessage returns the Message field value
func (o *BulkChargeFetchBulkBatchChargesResponse) GetMessage() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Message
}

// GetMessageOk returns a tuple with the Message field value
// and a boolean to check if the value has been set.
func (o *BulkChargeFetchBulkBatchChargesResponse) GetMessageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Message, true
}

// SetMessage sets field value
func (o *BulkChargeFetchBulkBatchChargesResponse) SetMessage(v string) {
	o.Message = v
}

// GetData returns the Data field value
func (o *BulkChargeFetchBulkBatchChargesResponse) GetData() []BulkChargeFetchBulkBatchChargesResponseArray {
	if o == nil {
		var ret []BulkChargeFetchBulkBatchChargesResponseArray
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *BulkChargeFetchBulkBatchChargesResponse) GetDataOk() ([]BulkChargeFetchBulkBatchChargesResponseArray, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *BulkChargeFetchBulkBatchChargesResponse) SetData(v []BulkChargeFetchBulkBatchChargesResponseArray) {
	o.Data = v
}

// GetMeta returns the Meta field value
func (o *BulkChargeFetchBulkBatchChargesResponse) GetMeta() BulkChargeFetchBulkBatchChargesResponseMeta {
	if o == nil {
		var ret BulkChargeFetchBulkBatchChargesResponseMeta
		return ret
	}

	return o.Meta
}

// GetMetaOk returns a tuple with the Meta field value
// and a boolean to check if the value has been set.
func (o *BulkChargeFetchBulkBatchChargesResponse) GetMetaOk() (*BulkChargeFetchBulkBatchChargesResponseMeta, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Meta, true
}

// SetMeta sets field value
func (o *BulkChargeFetchBulkBatchChargesResponse) SetMeta(v BulkChargeFetchBulkBatchChargesResponseMeta) {
	o.Meta = v
}

func (o BulkChargeFetchBulkBatchChargesResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BulkChargeFetchBulkBatchChargesResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["status"] = o.Status
	toSerialize["message"] = o.Message
	toSerialize["data"] = o.Data
	toSerialize["meta"] = o.Meta
	return toSerialize, nil
}

type NullableBulkChargeFetchBulkBatchChargesResponse struct {
	value *BulkChargeFetchBulkBatchChargesResponse
	isSet bool
}

func (v NullableBulkChargeFetchBulkBatchChargesResponse) Get() *BulkChargeFetchBulkBatchChargesResponse {
	return v.value
}

func (v *NullableBulkChargeFetchBulkBatchChargesResponse) Set(val *BulkChargeFetchBulkBatchChargesResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableBulkChargeFetchBulkBatchChargesResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableBulkChargeFetchBulkBatchChargesResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBulkChargeFetchBulkBatchChargesResponse(val *BulkChargeFetchBulkBatchChargesResponse) *NullableBulkChargeFetchBulkBatchChargesResponse {
	return &NullableBulkChargeFetchBulkBatchChargesResponse{value: val, isSet: true}
}

func (v NullableBulkChargeFetchBulkBatchChargesResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBulkChargeFetchBulkBatchChargesResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


