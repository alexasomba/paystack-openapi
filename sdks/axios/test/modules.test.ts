...
describe('Module Coverage Tests', () => {
	...
	const paystack = createPaystack({ secretKey, axiosInstance: mockAxios });

	beforeEach(() => {
		// eslint-disable-next-line @typescript-eslint/unbound-method
		const requestMock = vi.mocked(mockAxios.request);
		requestMock.mockReset();
		requestMock.mockResolvedValue({
			status: 200,
			data: Buffer.from(JSON.stringify({ status: true, data: {} })),
	...
	function assertCall(urlPart: string, method = 'GET') {
		...
		expect(config.url).toContain(urlPart);
		expect(config.method?.toUpperCase()).toBe(method.toUpperCase());
	}
