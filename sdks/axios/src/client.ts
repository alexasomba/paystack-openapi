...
export interface PaystackAxiosClientOptions {
  secretKey: string;
  baseUrl?: string;
...
function resolveIdempotencyMode(options: { idempotencyKey?: string | (() => string) }): IdempotencyKeyInput {
...
function wrapFetch(
	...
	return (async (input: RequestInfo | URL, init?: RequestInit) => {
		...
		const initWithIdempotency: RequestInit = {
			...init,
			method,
			headers: autoKey !== undefined ? setHeader(requestHeaders, idempotencyHeader, autoKey) : requestHeaders,
		};
...
export function createPaystackClient(options: PaystackAxiosClientOptions): PaystackClient {
	...
	const wrappedFetch = wrapFetch(axiosFetch, {
